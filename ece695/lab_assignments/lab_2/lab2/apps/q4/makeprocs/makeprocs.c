...
void main (int argc, char *argv[])
{
  ...
  cond_t s_fullslots;        // Cond_var used to wait for not full
  cond_t s_emptyslots;        // Cond_var used to wait for not empty
  ...
  if ((s_fullslots = cond_create(buffer_access_lock)) == SYNC_FAIL) {
    Printf("Bad cond_create 1 in "); Printf(argv[0]); Printf("\n");
    Exit();
  }

  if ((s_emptyslots = cond_create(buffer_access_lock)) == SYNC_FAIL) {
    Printf("Bad cond_create 2 in "); Printf(argv[0]); Printf("\n");
    Exit();
  }
  
  // Setting empty slots to True
