...
//
//	(Lab 1 Task)
//	This routine returns an integer representing the ID of the 
//	current process. The ID of a process in DLXOS can be defined
//	as simply the process control block's index in rge array of
...
unsigned int GetCurrentPid()
{
  ...
  dbprintf ('k', "Entering GetCurrentPid\n");
  current_pid = 0;
  dbprintf ('k', "The currentPCB is @ 0x%x.\n", (int)(currentPCB));
  dbprintf ('k', "The currentPCB->currentSavedFrame is 0x%x.\n", currentPCB->currentSavedFrame);
  dbprintf ('k', "The currentPCB->sysStackPtr is 0x%x.\n", currentPCB->sysStackPtr);
  dbprintf ('k', "The currentPCB->sysStackArea is 0x%x.\n", currentPCB->sysStackArea);
  dbprintf ('k', "The currentPCB->flags is %d.\n", currentPCB->flags);
  dbprintf ('k', "The currentPCB->pagetable is 0x%x.\n", currentPCB->pagetable);
  dbprintf ('k', "The currentPCB->npages is %d.\n", currentPCB->npages);
  for (i = 0; i < PROCESS_MAX_PROCS; i++) {
    dbprintf ('k', "Comparing PCB id %d @ 0x%x.\n", i, (int)&(pcbs[i]));
    if ((pcbs[i].currentSavedFrame == currentPCB->currentSavedFrame) && 
        (pcbs[i].sysStackPtr == currentPCB->sysStackPtr) &&
        (pcbs[i].sysStackArea == currentPCB->sysStackArea) &&
        (pcbs[i].flags == currentPCB->flags) &&
        //(pcbs[i].name == currentPCB->name) && //string cannot be compared easily, with this we are just comparing pointers to strings
        (pcbs[i].pagetable == currentPCB->pagetable) &&
        (pcbs[i].npages == currentPCB->npages)
        //(pcbs[i].l == currentPCB->l) //l is of type Link, which is again a user defined structure. Hence not comparing here.
       ) {
       current_pid = i;
       dbprintf ('k', "BINGO!! Found a match\n");
       dbprintf ('k', "The pcbs[i].currentSavedFrame is 0x%x.\n", pcbs[i].currentSavedFrame);
       dbprintf ('k', "The pcbs[i].sysStackPtr is 0x%x.\n", pcbs[i].sysStackPtr);
       dbprintf ('k', "The pcbs[i].sysStackArea is 0x%x.\n", pcbs[i].sysStackArea);
       dbprintf ('k', "The pcbs[i].flags is %d.\n", pcbs[i].flags);
       dbprintf ('k', "The pcbs[i].pagetable is 0x%x.\n", pcbs[i].pagetable);
       dbprintf ('k', "The pcbs[i].npages is %d.\n", pcbs[i].npages);
       break;
     }
  }
  dbprintf ('k', "Current PID is %d.\n", current_pid);
  dbprintf ('k', "Leaving GetCurrentPid\n");
  return current_pid;
}
