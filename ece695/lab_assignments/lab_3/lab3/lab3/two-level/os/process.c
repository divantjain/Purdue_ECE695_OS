...
void ProcessFreeResources (PCB *pcb) {
  ...
  for (i=0; i<MEM_L1_PTSIZE; i++) {
    if(pcb->pagetable[i] != 0x0) { // 0x0 is the OS address, so if the L1 PTE is valid, it must be non-zero
      MemoryFreeL2Pt(pcb->pagetable[i]);
      pcb->pagetable[i] = 0x0;
...
int ProcessFork (VoidFunc func, uint32 param, char *name, int isUser) {
  ...
  for (i=0; i<4; i++) {
    MemoryAllocForPageAndSetL1PtAndL2Pt(pcb, MEM_PAGESIZE*i);
  }
  ...
  if (isUser) {
    ...
    MemoryAllocForPageAndSetL1PtAndL2Pt(pcb,  MEM_PAGESIZE*(MEM_NUM_VIR_PAGES-1));
    stackframe[PROCESS_STACK_USER_STACKPOINTER] = (MEM_MAX_VIRTUAL_ADDRESS - 0x3); // MEM_MAX_VIRTUAL_ADDRESS - 0x3 
