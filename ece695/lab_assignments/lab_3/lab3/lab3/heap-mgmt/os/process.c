...
void ProcessModuleInit () {
  ...
  for (i = 0; i < PROCESS_MAX_PROCS; i++) {
    ...
    //-------------------------------------------------------
    for (j = 0; j < MEM_L1_PTSIZE; j++) {
      pcbs[i].pagetable[j] = 0x0;
    }
    pcbs[i].npages = 0;
    
    InitializeHeapTree(&(pcbs[i]));
...
void ProcessFreeResources (PCB *pcb) {
  ...
  //------------------------------------------------------------
  // Free the pages whose PTE_VALID bit is 1
  for (i=0; i<MEM_L1_PTSIZE; i++) {
    if((pcb->pagetable[i] & MEM_PTE_VALID) == 1) {
      MemoryFreePte(pcb->pagetable[i]);
      pcb->pagetable[i] = 0x0;
      //dbprintf('m', "Inside ProcessFreeResources: Freed L1 Page Table Entry: %d\n", i);
    }
  }
  ...
  // Deleting the heap_root for this process
  FreeHeapTree(pcb);
  //dbprintf('m', "Inside ProcessFreeResources: Freed Physical Page for system stack: %d\n", ((pcb->sysStackArea) / MEM_PAGESIZE));  
...
int ProcessFork (VoidFunc func, uint32 param, char *name, int isUser) {
  ...
  //---------------------------------------------------------
  // Allocating 4 pages for the Code and Global Variables
  pcb->npages = pcb->npages + 4;
  for (i=0; i<4; i++) {
    new_page = MemoryAllocPage();
    if (new_page == 0) {
      printf ("FATAL: couldn't allocate memory for Code [%d] - no free pages! Inside ProcessFork\n", i);
      exitsim ();	// NEVER RETURNS!
    }
    pcb->pagetable[i] = MemorySetupPte(new_page);
  }

  // Allocating 1 pages for Heap
  pcb->npages = pcb->npages + 1;
  new_page = MemoryAllocPage();
  if (new_page == 0) {
    printf ("FATAL: couldn't allocate memory for System Stack - no free pages! Inside ProcessFork\n");
    exitsim ();	// NEVER RETURNS!
  }
  pcb->pagetable[4] = MemorySetupPte(new_page);
  AssignHeapNode(&(pcb->heap_tree[0]), pcb->heap_tree[0].order, pcb->heap_tree[0].addr, pcb->heap_tree[0].size, 1, 0, 0, pcb->heap_tree[0].parent, pcb->heap_tree[0].left, pcb->heap_tree[0].right, pcb->heap_tree[0].buddy);
  //PrintHeapNode(&(pcb->heap_tree[0]));

  // Allocating 1 pages for System Stack and assigning the Page address to the SysStackArea
  pcb->npages = pcb->npages + 1;
  new_page = MemoryAllocPage();
  if (new_page == 0) {
    printf ("FATAL: couldn't allocate memory for System Stack - no free pages! Inside ProcessFork\n");
  ...
  //System Stack pointer now points to the last address of the physical page allocated to System Stack
  stackframe = ((uint32 *)(pcb->sysStackArea + MEM_PAGESIZE - 0x4));
  ...
  stackframe[PROCESS_STACK_PTSIZE] = MEM_L1_PTSIZE;
  ...
  if (isUser) {
    ...
    //----------------------------------------------------------------------
    // Allocating 1 pages for User Stack
    pcb->npages = pcb->npages + 1;
    new_page = MemoryAllocPage();
    if (new_page == 0) {
      printf ("FATAL: couldn't allocate memory for User Stack - no free pages! Inside ProcessFork\n");
      exitsim ();	// NEVER RETURNS!
    }
    pcb->pagetable[MEM_L1_PTSIZE-1] = MemorySetupPte(new_page);
    stackframe[PROCESS_STACK_USER_STACKPOINTER] = ((MEM_L1_PTSIZE * MEM_PAGESIZE) - (0x4)); // MEM_MAX_VIRTUAL_ADDRESS - 0x3 
